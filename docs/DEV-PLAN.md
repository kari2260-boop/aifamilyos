...
> 这部分是当前代码库的最新状态记录，后续如果继续开发，先从这里接着做。

### 评估结论

当前项目已经具备交付基础，不是从零开始的阶段。核心能力已经落地，包括：
- 4 个 AI Agent 对话、RAG 知识库、家庭档案、测评、咨询、月报、课程、文章、后台管理
- `subscription_plan` / `monthly_quota` / `assessment_quota` / `report_quota` 已进入数据模型
- 管理后台已支持家庭套餐开通、续费和到期查看
- 数据库连接池、全局异常处理、JWT 启动校验、基础限流已经有了

现在真正需要补的，不是继续堆页面，而是把“交付闭环”和“权益闭环”做实。

### 当前必须注意的点

1. 代码仓库的 Git 远程地址曾包含敏感 token，后续要先轮换凭据，再统一 remote 配置。
2. 部署脚本 `deploy.sh` 的仓库地址和本地当前 remote 不是同一个路径，部署前要先统一。
3. 流式对话接口里还存在配额判断问题，`monthly_quota = NULL` 的场景和结束事件里的剩余额度要统一处理。
4. 课程权限目前更多是前端展示层在挡，后端还需要补强校验，避免付费内容被绕过。
5. 测评流程已经能跑，但还没真正消耗 `assessment_quota`，也没有把 9.9 轻测评和 3480 / 9800 正式测评完全拆开。
6. 备份和恢复演练没有形成稳定脚本化流程，这个要在继续扩大用户前补上。

### 继续开发的优先级

#### P0：上线前必须修

- 统一 Git remote 和部署仓库来源
- 修复流式对话的配额判断和剩余额度返回
- 后端补课程权限校验，不只依赖前端锁
- 补数据库自动备份和恢复演练
- 确认生产部署链路能稳定跑通
...
- 把 `subscription_plan` 彻底变成唯一权益主字段
- 给家庭增加交付状态流转：未建档、已建档、已测评、待审核、已发布、待咨询、已完成
- 后台家庭总览补齐：套餐、到期、建档、测评、报告、AI 使用、活跃、备注、待办
- 给 9800 咨询和线下营增加核销字段
- 批量导入已有客户，支持手机号、产品等级、来源、开通时间

#### P2：支撑 9.9 转化

- 课程增加产品权限字段，支持体验课、正式课、专属课
- 9.9 的有效期、AI 额度和升级入口统一成一套规则
- 建高意向评分和跟进列表
- 给后台加“待跟进 / 已联系 / 有意向 / 已转化”状态

### 当前代码事实

- `backend/app/routers/subscription.py` 已经定义了 `free / trial_9_9 / community_3480 / pilot_9800` 三档产品逻辑。
- `backend/app/models/models.py` 和对应迁移已经把家庭订阅字段加进来了。
- `backend/app/routers/admin.py` 已经能在管理后台查看家庭的套餐和到期时间。
- `backend/app/routers/chat.py` 还需要再检查一次配额边界，尤其是不限量套餐和流式返回。
- `backend/app/routers/course.py` 已经有课程列表和详情，但还没把“谁能看什么”做成真正的后端规则。
- `backend/app/routers/assessment.py` 已经能导入、提交和出报告，但还缺权益消耗和产品分层。

### 这份文档后续怎么用

后续新开任务时，先按这个顺序看：
1. 先看这里的“最新评估记录”
2. 再看 `docs/DEV-PLAN.md` 里的 Phase
3. 再对照 `docs/PRD-v3.md` 的权益定义
4. 最后按当前代码事实补实现，不要只按旧文档推断
