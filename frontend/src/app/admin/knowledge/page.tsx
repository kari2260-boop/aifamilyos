  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="bg-card px-4 py-3 flex items-center gap-3 border-b border-border sticky top-0 z-10">
        ...
        <h1 className="font-semibold text-foreground">知识库管理</h1>
      </div>
      ...
      <div className="px-4 space-y-3">
        {docs.map((doc) => (
          <div key={doc.id} className="bg-card rounded-xl p-4 shadow-sm border border-border">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium text-foreground truncate flex-1">{doc.title}</h3>
        ...
        {docs.length === 0 && (
          <p className="text-center text-muted-foreground/60 text-sm mt-6">暂无文档</p>
        )}
      </div>
    </div>
  );
